...
def send_welcome(message):
    ...
    if check_admin(id) == False:
        ...
        keyboard.add(item1)
        keyboard.add(item2)     
        send = bot.send_message(message.chat.id, "Welcome! Approved.. Cracked By 👸@sana88999 \n 🥳\n\nWould you like to be in user or admin mode?",
...
def send_text(message):
    ...
    try:
        ...
        bot.send_message(chat_id, "An error has occured , You are Noob")
...
def how_to_help(message):
    bot.send_message(message.chat.id, "• Do Not Contact Anyone Noob \n\n")


@bot.message_handler(commands=['faq'])
def how_faq(message):
    bot.send_message(message.chat.id, "Noob Don't Again Click here...")
